...
- `20251224223000_root_cause_fix.sql` shows duplicate cleanup logic
...
**Priority:** 🟠 **HIGH - Plan for next major version**
...
CREATE TABLE public.bookings (
  -- ... other fields ...
  total_price numeric NOT NULL,           -- ✓ Acceptable (transaction snapshot)
  platform_commission numeric,            -- ⚠️ Should be calculated
  partner_revenue numeric,                -- ⚠️ Should be calculated
  refund_amount numeric,                  -- ⚠️ Duplicates booking_cancellations
  refund_timestamp timestamp,             -- ⚠️ Duplicates booking_cancellations
  -- ...
);
```
...
**Priority:** 🟡 **MEDIUM - Clarify intent, then implement**
...
CREATE TABLE public.passengers (
  ...
  trip_id bigint,         -- ⚠️ REDUNDANT - derivable from booking
  seat_id bigint,
  -- ...
);
```
...
**Priority:** 🟢 **LOW - Optimize during refactoring**
...
**Priority:** 🟠 **HIGH - Clarify immediately to prevent data inconsistency**
...
The `20251224223000_root_cause_fix.sql` migration reveals a critical design flaw:
...
**Priority:** 🔴 **CRITICAL - Clarify business requirement immediately**
...
CREATE TABLE public.buses (
  ...
  owner_user_id bigint,     -- ⚠️ Individual ownership?
  -- ...
);
```
...
**Priority:** 🟡 **MEDIUM - Clarify business logic**
...
#### 3.2.3 Circular Dependency: Bookings ↔ Passengers
...
- Booking → Passengers → Cancellation → Booking
...
**Priority:** 🟢 **LOW - Document and validate logic**
...
CREATE TABLE public.commissions (
  ...
  booking_id bigint,
  partner_id bigint,
  trip_id bigint,          -- ⚠️ Redundant with booking→trip
  -- ...
);
```

**Issue:** `trip_id` is redundant (derivable from `booking_id → trip_id`)
...
#### 3.3.1 Missing Junction Table: Buses ↔ Drivers
...
**Priority:** 🟡 **MEDIUM - Depends on business requirement**
...
**Priority:** 🔴 **CRITICAL - Implement immediately for production**
...
**Priority:** 🟠 **HIGH - Optimize before scaling**
...
CREATE TABLE seats (
  ...
  bus_id bigint,
  seat_number varchar,
  is_available boolean DEFAULT true  -- ⚠️ Global flag, not trip-specific
);
```
...
**Priority:** 🔴 **CRITICAL - Fix immediately (data model flaw)**
...
**Priority:** 🟡 **MEDIUM - Implement before 100K bookings**
...
**Priority:** 🟡 **MEDIUM - Implement before 1M notifications**
...
✅ Partner data isolation implemented  
...
|-------|------|----------------|
| `buses` | ❌ No RLS | Enable partner isolation |
| `seats` | ❌ No RLS | Enable (public read acceptable) |
| `users` | ❌ No RLS | Enable user self-access only |
| `employees` | ✅ Has RLS | OK |
| `payment_transactions` | ❌ No RLS | Enable user/admin access only |
| `refunds` | ❌ No RLS | Enable user/admin access only |
| `refund_transactions` | ❌ No RLS | Enable admin access only |
| `documents` | ❌ No RLS | Enable user/partner/admin access |
| `conversations` | ❌ No RLS | Enable user self-access only |
| `messages` | ❌ No RLS | Enable conversation participant access |
...
**Priority:** 🟠 **HIGH - Prevent invalid data entry**
...
✅ `role_changes_log` (from migration 20251224231000)  
✅ `booking_approvals` (tracks approval actions)  
...
✅ Sophisticated cancellation policy system  
✅ Multi-tenant architecture with partner isolation  

### Key Weaknesses:
❌ No foreign key constraints (catastrophic for data integrity)  
❌ No indexes beyond primary keys (performance disaster at scale)  
❌ Flawed seat availability model (business logic broken)  
❌ Incomplete RLS coverage (security gaps)  
❌ Missing data validation constraints (allows invalid data)  
