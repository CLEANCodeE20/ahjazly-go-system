-- ========================================================
-- TRIP-SPECIFIC SEAT BLOCKING
-- ========================================================

-- 1. Add blocked_seats to trips
ALTER TABLE public.trips 
ADD COLUMN IF NOT EXISTS blocked_seats JSONB DEFAULT '[]'::jsonb;

-- 2. Update get_available_seats RPC to include blocked seats logic
...
DECLARE
    ...
    v_seats JSONB;
BEGIN
    -- 1. Get bus_id and blocked_seats for the trip
    SELECT bus_id, COALESCE(blocked_seats, '[]'::jsonb) 
    INTO v_bus_id, v_blocked_seats 
    FROM public.trips 
    WHERE trip_id = p_trip_id;
    
    -- 2. Fetch all seats for this bus, marking them as taken if a passenger exists OR if the seat is blocked
    SELECT jsonb_agg(
        jsonb_build_object(
            'seat_id', s.seat_id,
...
-- 3. Update search_trips RPC to subtract blocked seats from availability count
    ...
    SELECT
        ...
        (
            ...
            (
                ...
                WHERE pass.trip_id = t.trip_id AND pass.passenger_status = 'active'
            )
            -
            (
