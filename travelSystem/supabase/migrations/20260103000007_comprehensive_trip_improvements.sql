    ...
    WHERE
        ...
        AND (
            (
                SELECT COUNT(*) 
                FROM public.seats s
                WHERE s.bus_id = t.bus_id AND s.is_available = true
            ) 
            - 
            (
                SELECT COUNT(*) 
...
BEGIN
    ...
    SELECT 
        (
            SELECT COUNT(*) 
            FROM public.seats s
            WHERE s.bus_id = v_trip.bus_id AND s.is_available = true
        ) 
        - 
        (
            SELECT COUNT(*) 
            FROM public.passengers pass 
            WHERE pass.trip_id = p_trip_id AND pass.passenger_status = 'active'
        )
        -
        (
            SELECT jsonb_array_length(COALESCE(blocked_seats, '[]'::jsonb))
            FROM public.trips
            WHERE trip_id = p_trip_id
        )
