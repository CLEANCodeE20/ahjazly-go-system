...
BEGIN
    ...
    SET 
        ...
        cancel_reason = CASE WHEN p_new_status = 'cancelled' THEN p_notes ELSE cancel_reason END,
        cancel_timestamp = CASE WHEN p_new_status = 'cancelled' THEN now() ELSE cancel_timestamp END
    WHERE booking_id = p_booking_id;
    ...
    INSERT INTO public.booking_approvals (
        ...
        CASE 
            ...
            WHEN p_new_status::TEXT = 'cancelled' THEN 'reject'
            ELSE 'update'
        END,
        v_old_status,
        p_new_status::TEXT,
